...
kde_poly = readOGR("kde_3857_final.sqlite", GDAL1_integer64_policy = TRUE)
species_pts = readOGR("pres_pts3857.sqlite")
...
sp_dt = sp_dt[order(sp_dt$Est_Suitable_Area_Acres, decreasing = T),]
...
checkboxGroupButtons(inputId = "mgmt.admin", direction = 'vertical', choices = c('BLM Districts', 'EPMT', "NPS", 'U.S. Counties'), 
                     status = "primary", individual = T, size = 'sm', selected = 'BLM Districts', 
...
shinyApp(ui = fillPage(tags$style(type = "text/css",
  ...
  output$map = renderLeaflet({
    leaflet(options = leafletOptions(maxZoom = 10)) %>%
      ...
      addPolygons(data = states, fill = 0, weight = 1.35, group = "states") %>%
  ...
  observe({
    ...
    if(!is.null(d)){
      leafletProxy("map") %>% 
        clearGroup( 'distro') %>%
        addPolygons(data=distro_shp(), group='distro', color = "green", fill = 0)
    } else {
      leafletProxy("map") %>%
        clearGroup('distro')
      }
  ...
  observe({
    ...
    if(!is.null(p)){
      leafletProxy("map") %>%
        clearGroup( 'pres_pts') %>%
        addCircleMarkers(data=pres_pts(), group='pres_pts', color = "purple", radius = 0.3)
    } else {
        leafletProxy("map") %>%
          clearGroup( 'pres_pts')
      }
...
fillPage(DT::renderDataTable(DT::datatable(sp_dt
              , fillContainer = getOption("DT.fillContainer", TRUE)
              , caption = "Summary information by National Park for a selected species and EPMT region. 'Known presence' indicates if presence locations from the park were available for model development. 'Established suitable area' is the number of acres suitable when suitability values between 0 and 1 were transformed to unsuitable and suitable categories by allowing 1% of the known locations to be classified as unsuitable. 'Percent of Park' is the percent of the park area that is classified as suitable. 'Minimum distance to Occurrence' is the minimum distance from the park boundary to a known occurrence used in model development. Boundaries generated from NPS IRMA (https://irma.nps.gov/DataStore/Reference/Profile/2224545?lnv=True)"
              , colnames = c("Species", "EPMT Unit", "National Park", "Known Presence? (Count)"
                             ,"Estimated Suitable Area (Acres)", "Percent of Park Area"
